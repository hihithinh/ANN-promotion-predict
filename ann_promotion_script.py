...
if train_df is not None:
    ...
    print(f"Resampled y_train distribution after SMOTE: {np.bincount(y_train_smote)}")
    ...
    # Training data uses SMOTE resampled data
...
def compute_cost(AL, Y):
    ...
    cost = (-1/m) * np.sum(Y * np.log(AL + 1e-8) + (1-Y) * np.log(1-AL + 1e-8)) # Added 1e-8 for numerical stability
...
def L_model_backward(AL, Y, caches, activations_list):
    ...
    for l in reversed(range(L-1)):
        ...
        dA_prev_temp, dW_temp, db_temp = linear_activation_backward(grads["dA" + str(l+1)], current_cache, activations_list[l])
...
def nn_model(X_train, Y_train, X_val, Y_val, layers_dims, activations_list, learning_rate=0.0075, num_iterations=3000, print_cost=False, print_cost_every=100):
    """
